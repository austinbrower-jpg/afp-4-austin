...
export async function GET() {
  try {
    ...
    const operationalHours = clientHours.filter((entry) => !isQuarantinedRecord(entry));
    const week = getWeekRange();
    const month = getMonthRange();
    const latestInvoice = invoices.sort((a, b) => b.periodEnd.localeCompare(a.periodEnd))[0] ?? null;
    const unbilled = latestInvoice ? operationalHours.filter((entry) => entry.date > latestInvoice.periodEnd) : operationalHours;
    const activeProjects = projects.filter((project) => project.status === "active");
    const activeProject = activeProjects.sort((a, b) => {
      const aEntry = operationalHours.find((entry) => entry.projectId === a.id) ?? null;
      const bEntry = operationalHours.find((entry) => entry.projectId === b.id) ?? null;
      if (aEntry && !bEntry) return -1;
      if (bEntry && !aEntry) return 1;
      const aDate = aEntry?.date ?? a.updatedAt;
      const bDate = bEntry?.date ?? b.updatedAt;
    ...
    const payload: DashboardSummary = {
      ...
      client,
      today: { hours: sumHours(entriesToday(operationalHours)), date: todayISO() },
      week: { hours: sumHours(entriesInRange(operationalHours, week.start, week.end)), start: week.start.toISOString().slice(0, 10), end: week.end.toISOString().slice(0, 10) },
      month: { hours: sumHours(entriesInRange(operationalHours, month.start, month.end)), start: month.start.toISOString().slice(0, 10), end: month.end.toISOString().slice(0, 10) },
