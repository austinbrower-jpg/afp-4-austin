...
export function formatApiError(error: unknown, fallback?: string): { body: ApiErrorBody; status: number } {
  ...
  const message = fallback ?? (error instanceof Error && error.message ? error.message : "Unexpected API error.");
